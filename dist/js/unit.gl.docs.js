...
function initDeviceDemo() {
  ...
  function updateUI() {
    ...
    const filterEl2 = document.getElementById("dev-filter");
    const filterValue = String(filterEl2?.value || "").trim().toLowerCase();
    let matches = 0;
...
function initDensityDemo() {
  ...
  function updateDeviceInfo() {
    ...
    setText("current-dpr", dpr.toFixed(2) + "\xD7");
...
function initHybridScaleDemo() {
  ...
  function updateScale(value) {
    ...
    if (lcmMultiple) lcmMultiple.textContent = lcmVal / 20 + "\xD7 LCM";
    if (lcmAligned) {
...
//# sourceMappingURL=unit.gl.docs.js.map