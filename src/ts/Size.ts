...
export default class Size {
    ...
    constructor(width: Unit, height: Unit) {
        if (width.unit !== height.unit) {
            throw new Error(
                "Width and height must have the same unit"
            );
        }
        this.width = width;
        this.height = height;
    ...
    subtract(other: Size): Size {
        return new Size(
            this.width.subtract(other.width),
            this.height.subtract(other.height)
        );
    }
    ...
    convert(toUnit: UnitType): Size {
        return new Size(
            this.width.convert(toUnit),
            this.height.convert(toUnit)
        );
    }
